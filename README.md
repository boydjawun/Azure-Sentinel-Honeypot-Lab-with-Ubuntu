...
The geolocation and map.json files used in this project were provided by Josh Madakor’s Azure SOC Lab YouTube Tutorial (https://www.youtube.com/watch?v=g5JL2RIbThM), to set up the Log Analytics Workshop and map in Sentinel to reveal where the attackers that are trying to attack the Virtual Machine are located. Be sure to turn your running machines off to avoid unwanted charges from Azure when the project is finished!
...
- Create a Resource Group
    > Resource Group - A container that helps organize and manage related cloud resources
    - Type "Resource Group" in the search bar and fill in:
      - The subscription type
      - The name of the resource group
      - The region
- After creating the resource group, click review + create then click create after reviewing the details of the resource group
...
- Currently, nothing is inside the resource groups container thats just been created.
...
- This actual honeypot is exposed to the internet for hackers to attack.
...
*I used the Ubuntu OS for the Virtual Machine for free tier users. Be sure to turn off the virtual machine when done to avoid charges*
...
- Scroll down to check the checkbox to delete the IP and NIC(Mac Address) when IP is done, and the Virtual Machine is shut off.
...
- Edit the NSG to open the firewall up to the internet, click the NSG link in the resource group
...
- Go to the right hand corner and delete the SSH rule by clicking the trashcan.
...
- Go to the connect tab and click the check access button which provides a ssh script that is used to connect with the Ubuntu VM using the Windows Command prompt on your computer. Also, use the path to the ssh key password file downloaded previously to access the ssh, the file should be a ***.pem*** file.
  
  ```ssh -i [private-key-file-path].pem azureuser@20.161.184.9```
...
- ```sudo tail -f /var/log/auth.log``` shows all of the logs for the Ubuntu machine, like event viewer in Windows.
...
- Click review + create, wait, then click create.
...
- Now there is a connection from our Log workspace to Sentinel. The VM has to be connected to the Log Analytics Workspace.
...
- Go to the content management tab, then content hub in Sentinel.
...
- Use KQL to filter out everything you want to see in this workspace:
...
- Download [geoip-summarized.csv](https://drive.google.com/file/d/13EfjM_4BohrmaxqXZLB5VUBIz2sv9Siz/view?usp=sharing) that shows random IP Addresses and their locations, and save it to your PC.
...
- Fill in source information and use the downloaded csv.
...
- Get a random IP address from a Hacker.
...
- Head to Sentinel, click on the Log Analytics instance, then click workbook under threat management.
...
- Go to advanced editor tab, delete the code already in there, and paste the text from the json file and click “apply”.
